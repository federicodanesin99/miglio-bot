...
function makeSender(session, cfg) {
  ...
  async function sendThrottled(jid, text, mentions = null) {
    ...
    lastSendAt = Date.now();
    return sent; // include .key — serve a chi deve agganciare reazioni (es. /presente)
  }
...
// `rollcall` (opzionale) aggrega le reazioni 👍 all'appello /presente.
function makeInboundRouter({ cfg, sender, ctxFactory, rollcall = null }) {
  ...
  return async (upsert) => {
    ...
    for (const msg of upsert.messages || []) {
      try {
        ...
        // Reazioni: aggregazione silenziosa per l'appello in corso (anche fromMe? no).
        const reaction = msg.message.reactionMessage;
        if (reaction) {
          if (rollcall && !msg.key.fromMe && rollcall.matchesKey(reaction.key)) {
            const reactor = msg.key.participant || msg.key.remoteJid;
            rollcall.record(reactor, reaction.text || '');
          }
          continue;
        }
        ...
        // Metadati per-messaggio per i comandi che ne hanno bisogno (es. /mvp:
        // chi vota e chi è menzionato). I path di test passano meta vuoto.
        const voter = msg.key.participant || msg.key.remoteJid;
        const mentions =
          msg.message.extendedTextMessage?.contextInfo?.mentionedJid || [];
        const pushName = msg.pushName || '';
        const ts = (Number(msg.messageTimestamp) || 0) * 1000;
        const ctx = ctxFactory({ voter, mentions, pushName, ts });
        if (ctx.mvp && pushName) ctx.mvp.noteName(voter, pushName);

        const reply = await dispatch(text, ctx);
        if (!reply) continue;
        // Un comando può rispondere con una stringa, o con { text, after(sent) }
        // per agire sul messaggio appena inviato (es. /presente aggancia le reazioni).
        if (typeof reply === 'string') {
          await sender.sendThrottled(chatId, reply);
        } else if (reply.text) {
          const sent = await sender.sendThrottled(chatId, reply.text, reply.mentions || null);
          if (reply.after) await reply.after(sent);
        }
      } catch (e) {
