...
const { makeHypePicker } = require('./hype');
...
function buildEvents(cfg, filterDay = null, deps = {}) {
  ...
  // Messaggi "hype" per gasare il gruppo. Ogni slot con `text` è FISSO (es. la
  // FOTO DI GRUPPO); senza `text` è "a sorpresa": pesca dal pool al momento
  // dell'invio (build()), così varia e non si ripete. Id deterministici
  // `hype:<slot.id>` → idempotenza (ogni slot parte una volta sola, anche dopo
  // riavvii). Un solo picker condiviso per ridurre i doppioni nello stesso run.
  if (cfg.hype && cfg.hype.enabled) {
    const pick = makeHypePicker(cfg.hype.pool || []);
    for (const slot of cfg.hype.slots || []) {
      events.push({
        id: `hype:${slot.id}`,
        type: 'hype',
        when: parseDayTime(slot.day, slot.time),
        target,
        text: slot.text || '🎲 Messaggio hype a sorpresa (scelto live dal pool).',
        build: slot.text ? undefined : async () => pick(),
      });
    }
  }
