...
function buildEvents(cfg, filterDay = null) {
  ...
  // Brief meteo mattutino: un evento per giorno. Il testo NON è statico — viene
  // calcolato al momento dell'invio via `build()` (open-meteo, previsione fresca).
  // `text` è solo un'anteprima per i dry-run offline (schedule/next).
  ...
  if (wx && wx.enabled) {
    ...
    for (const day of days) {
      events.push({
        ...
        target,
        text: `🌤️ Brief meteo ${place} — ${dayLabel(day)} (generato live al momento dell'invio)`,
        build: () => weatherBriefText(wx, day, cfg.trip.timezone),
      });
    }
  }
